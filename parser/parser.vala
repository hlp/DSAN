...
public class Parser : Object {
    ...
    Parameter? parse_parameter() {
        ...
        if (!accept(Token.ID)) // get variable name
            return null;
        ...
        Parameter p = new Parameter(type, name, source, scanner.start, scanner.end);

        if (accept(Token.EQUALS) || accept(Token.PIPE))
            skip_expression();
        return p;
    }
    ...
    Method? parse_method(Method m, string input) {
        ...
        // Look for a semicolon or left brace.  (There may be a pattern match)
        Token t = Token.NONE;
        do {
            t = next_token();
            if (t == Token.EOF)
    ...
    Symbol? parse_method_or_field(Class? enclosing_class) {
        bool parse_signal = false, parse_delegate = false, is_constructor = false, is_method = false;
        ...
        if (accept(Token.EXTERNAL)) {
            skip();
            return null;
        }

        if (accept(Token.CONSTRUCTOR))
            is_constructor = true;

        if (accept(Token.DEF)) // we actually don't neet to set this to know it is a method
            is_method = true;  // they're uniquely identified by the parenthesis
        
        if (enclosing_class != null && is_constructor && (peek_token() == Token.LEFT_PAREN))
        ...
        if (enclosing_class != null && is_constructor)
            return parse_method(new Constructor(name, enclosing_class, source), input);
        ...
        switch (peek_token()) {
            case Token.SEMICOLON:
            case Token.EQUALS:
                Field f = new Field(type, name, source, scanner.start, 0);
                skip();
                f.end = scanner.end;
                return f;
            case Token.LEFT_PAREN:
                Method m;
                if (parse_signal)
                    m = new VSignal(name, type, source);
                else if (parse_delegate)
                    m = new Delegate(name, type, source);
                else m = new Method(name, type, source);
                return parse_method(m, input);
            case Token.LEFT_BRACE:
                Property p = new Property(type, name, source, scanner.start, 0);
                next_token();
                p.body = parse_block();
                p.end = scanner.end;
                return p;
...
Gee.ArrayList<string> get_files_from_args(string[] args) {
    Gee.ArrayList<string> fnames = new Gee.ArrayList<string>();
...
string get_merged_files(Gee.ArrayList<string> filenames) throws FileError {
    string merged = "";

    foreach (string filename in filenames) {
        string source;
        if (!FileUtils.get_contents(filename, out source))
            throw new FileError.FAILED("Can't read file");
        merged += source;
    }
...
void main(string[] args) {
    ...
    Gee.ArrayList<string> filenames = get_files_from_args(args);
    string merged;
    try {
        merged = get_merged_files(filenames);
    } catch (FileError e) {
    ...
    new Valencia.Parser().parse(sf, merged);
    sf.print(0);
}
