...
public abstract class Variable : Symbol {
    ...
    public override void print(int level) {
        stdout.printf("{\n");
        //stdout.printf("\"type\" : \"parameter\",");
        if (type != null)
            stdout.printf("\"isa\" : \"%s\",\n", type.to_string());
        stdout.printf("\"name\" : \"%s\",\n", name);
        stdout.printf("\"type\" : \"variable\"\n");
        stdout.printf("}\n");
    }
}
...
public class Method : Symbol, Scope {
    ...
    protected virtual void print_type(int level) {
        stdout.printf("\"type\" : \"method\"\n");
    }
    
    private void json_start() {
        stdout.printf("{\n");
    }
    private void json_end() {
        stdout.printf("}\n");
    }
    ...
    public override void print(int level) {
        ...
        if (name != null)
            stdout.printf("\"name\" : \"%s\",\n", name);
        if (return_type != null)
            stdout.printf("\"returns\" : \"%s\",\n", return_type.to_string());
        
        if (!parameters.is_empty) {
            stdout.printf("\"parameters\" : [\n");
            for (int i = 0; i < parameters.size; ++i) {
                parameters.get(i).print(level + 1);
                if (i != parameters.size - 1) stdout.printf(",");
            }
            stdout.printf("],\n");
        }
        print_type(level);
...
public class Constructor : Method {
    ...
    public override void print_type(int level) {
        stdout.printf("\"type\" : \"constructor\"\n");
    }
}
    ...
    public override void print(int level) {
        ...
        if (!super.is_empty) {
            ...
            for (int i = 0 ; i < super.size ; ++i) {
                stdout.printf("\"%s\"", super.get(i).to_string());
                if (i != super.size - 1) stdout.printf(",\n");
            }
            stdout.printf("],\n");
        }
        
        if (!members.is_empty) {
            stdout.printf("\"members\" : [\n");
            for (int i = 0; i < members.size; ++i) {
                members.get(i).print(level + 1);
                if (i != members.size - 1) stdout.printf(",");
            }
            stdout.printf("],\n");
        }
        
        stdout.printf("\"name\" : \"%s\",", name);
        stdout.printf("\"type\" : \"class\"");
        stdout.printf("}\n");
    }
...
public class Namespace : TypeSymbol, Scope {
    ...
    public override void print(int level) {
        ...
        for (int i = 0; i < symbols.size; ++i) {
            symbols.get(i).print(level + 1);
            if (i != symbols.size - 1) stdout.printf(",");
        }
    }
}
...
public class SourceFile : Node, Scope {
    ...
    private void json_open() {
        stdout.printf("{\n    \"objects\" : [\n");
    }

    private void json_close() {
        stdout.printf("\n    ]\n}");
    }
